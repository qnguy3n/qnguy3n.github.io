...
related_posts: false
---
When writing up the manuscript for this [paper](https://pmc.ncbi.nlm.nih.gov/articles/PMC12919464), one question kept nagging me: **Is *Federated Averaging (FedAvg)* essentially just *Centralised Gradient Descent (CGD)* in disguise?** At the surface level, they look almost identical.
...
In FedAvg, the data stays strictly on-site. The central server broadcasts the global weights $w_t$ to each hospital. Each client $k$ then computes a local update:
...
Because $\sum_{k=1}^K p_k=1$, we can pull $w_t$ out of the summation, thus recovering the exact same equation as CGD.
...
In CGD, the server calculates a new global intermediate model ($w_{t,1}$) after the first step, and evaluates the second gradient at that shared, consensus position:

$$
w_{t+1}=w_t-\eta\sum_{k=1}^K p_k\nabla F_k(w_t)-\eta\sum_{k=1}^K p_k\nabla F_k(w_{t,1})
$$

In FedAvg, however, there is no shared consensus after step one. Client $k$ takes the first step, calculates a new local intermediate model ($w_{t,1}^k$), and evaluates the second gradient from that isolated position. When the server finally aggregates the models after two steps, the expanded equation the becomes:

$$
w_{t+1}=w_t-\eta\sum_{k=1}^K p_k\nabla F_k(w_t)-\eta\sum_{k=1}^K p_k\nabla F_k(w_{t,1}^k)
$$

The crucial difference lies in that final term. In CGD, the gradient $\nabla F_k$ is evaluated at $w_{t,1}$. In FedAvg, it is evaluated at $w_{t,1}^k$.
